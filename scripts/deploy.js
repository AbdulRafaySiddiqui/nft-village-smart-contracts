...
async function main() {
  ...
  await (
    await projectHandler.connect(deployer).addPool(0, pool, rewardInfo, requiredCards, { value: POOL_FEE })
  ).wait();
  ...
  // for test
  await (await testToken.approve(chief.address, constants.MaxUint256)).wait();
  console.log("approved");
  await (await testToken2.approve(chief.address, constants.MaxUint256)).wait();
  console.log("points approved");
