...
contract NFTVillageExchange is ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  function setRelayer(Relayer memory relayer) public onlyOwner {
    require(relayer.relayerAddress != address(0), "NFTVillageExchange: relayerAddress zero address");
    require(relayer.protocolFeeRecipient != address(0), "NFTVillageExchange: protocolFeeRecipient zero address");
   ...
   */
  function changeProtocolFeeRecipient(address relayer, address newProtocolFeeRecipient) public onlyOwner {
  ...
  function executeFundsTransfer(Order memory buy, Order memory sell) internal returns (uint256) {
    ...
    /** If ERC20 token payment method is used */
      ...
      IERC20Upgradeable(sell.paymentToken).safeTransferFrom(buy.maker, sell.maker, price - totalFee);
    }
  ...
  function chargeFee(
    ...
    if (relayerInfo.protocolFee != 0) {
      ...
      if (sellOrder.paymentToken == address(0))
        ...
        IERC20Upgradeable(sellOrder.paymentToken).safeTransferFrom(buyer, relayerInfo.protocolFeeRecipient, feeAmount);
    }
    ...
    if (relayerInfo.relayerFee != 0 && tokenDiscount != 10000) {
      ...
      if (sellOrder.paymentToken == address(0))
        ...
        /** Token as payment method */
      else IERC20Upgradeable(sellOrder.paymentToken).safeTransferFrom(buyer, sellOrder.relayer, feeAmount);
    }
    ...
    if (sellOrder.asset.supportsInterface(_INTERFACE_ID_FEES)) {
      ...
      if (recipients.length > 0) {
        for (uint256 i = 0; i < recipients.length; i++) {
          ...
          else IERC20Upgradeable(sellOrder.paymentToken).safeTransferFrom(buyer, recipients[i], fees[i]);
        }
      }
    }
    return totalFee;
  ...
  function payReferrerCommission(
    ...
    if (referrer != address(0)) {
      ...
      else IERC20Upgradeable(paymentToken).safeTransferFrom(buyer, referrer, referrerCommission);
    }
  ...
  function executeAssetTransfer(
    ...
    if (assetType == AssetType.ERC721) {
      IERC721Upgradeable(asset).safeTransferFrom(from, to, assetId);
    } else {
      IERC1155Upgradeable(asset).safeTransferFrom(from, to, assetId, assetAmount, "");
    }
  }
